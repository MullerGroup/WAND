...
static uint16_t reg_data_buf[100];
...
//	accel_init();
	...
	while(1)
	{
		...
		if (gui_request_counter>2000)
		{
			...
			MSS_SPI_transfer_block( &g_mss_spi0, ((uint8_t *) reg_data_buf) + 1, 199, spi_rx_buf, sizeof(spi_rx_buf));
        ...
        if ((stat & NM0_DATA) && (stat & NM1_DATA)) {
			...
			if ((crcflag0 & 0x1) && (crcflag1 & 0x1) && stream_en)
			{
				...
				if (sample_bool) {
					...
					adc_data_buf2[97] = counter;
					adc_data_buf2[98] = counter;
					adc_data_buf2[99] = counter;
				}
				else
				{
					// Accelerometer channels used as glitch finder
					adc_data_buf1[97] = counter;
					adc_data_buf1[98] = counter;
					adc_data_buf1[99] = counter;
				}
				MSS_SPI_set_slave_select( &g_mss_spi0, MSS_SPI_SLAVE_0 );
				if (sample_bool)
					MSS_SPI_transfer_block( &g_mss_spi0, ((uint8_t *) adc_data_buf2) + 1, 199, spi_rx_buf, sizeof(spi_rx_buf));
				else
					MSS_SPI_transfer_block( &g_mss_spi0, ((uint8_t *) adc_data_buf1) + 1, 199, spi_rx_buf, sizeof(spi_rx_buf));
				MSS_SPI_clear_slave_select( &g_mss_spi0, MSS_SPI_SLAVE_0 );
				spi_rx_handler();
			}
			else
