...
// needs 1 extra word for the header, 3 extra words for the accelerometer data
// we need 2 buffers since we are using double buffering to support artifact cancellation
static uint16_t adc_data_buf1[DATA_BUF_SIZE];
static uint16_t adc_data_buf2[DATA_BUF_SIZE];
...
//static bool test_comm_en = 0; // doesn't do anything now, but can be used to trigger hard-coded stim, for example.
//static bool test_comm_on = 0;
//static bool test_comm_off = 0;
...
static bool artifact_flag = 0; // whether we need to do artifact cancellation this sample or not
static int chan = 1;
...
//	accel_init();
	...
	writeNMreg(0, NM_REC_ENABLE0_REG, 0x0003);
	writeNMreg(0, NM_REC_ENABLE1_REG, 0x0000);
	...
	while(1)
	{
        ...
        if ((stat & NM0_DATA) && (stat & NM1_DATA)) {
			...
			if ((crcflag0 & 0x1) && (crcflag1 & 0x1) && stream_en)
			{
				...
				if (sample_bool) {
					// Accelerometer channels used as glitch finder
					adc_data_buf2[DATA_BUF_SIZE-3] = 100;
					adc_data_buf2[DATA_BUF_SIZE-2] = counter;
					adc_data_buf2[DATA_BUF_SIZE-1] = counter;
				}
				else
				{
					// Accelerometer channels used as glitch finder
					adc_data_buf1[DATA_BUF_SIZE-3] = 100;
					adc_data_buf1[DATA_BUF_SIZE-2] = counter;
...
//			if ((!stim_flag && !prev_stim) || !artifact_en)
