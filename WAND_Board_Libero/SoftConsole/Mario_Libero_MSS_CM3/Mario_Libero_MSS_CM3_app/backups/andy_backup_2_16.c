...
// we need 2 buffers since we are using double buffering to support artifact cancellation
static volatile uint16_t adc_data_buf1[DATA_BUF_SIZE];
static volatile uint16_t adc_data_buf2[DATA_BUF_SIZE];
...
static bool prev_stim = 0; // there was stim on previous sample
static int chan = 1;
static int dummy = 0; // for reading out NM adc FIFO data on a non-enabled channel
static bool sample_bool = 1; // For double buffering. True is populate buf1 send buf2, false is populate buf2 send buf1.
...
void fft_cl(int16_t sample)
{
	...
	if (cl0_en || cl1_en)
	{
		if (!rand_mode)
		{
			...
			fft_sample = (q15_t) (sample - mean) << 2;
			...
			// load samples to send to the appropriate buffer (2 bins, each has real, complex, and mag data)
			// must load these in the correct adc_data_buf for double buffering needed for artifact cancellation
			if (sample_bool)
			{
				adc_data_buf2[3] = (uint16_t) (mag_data_buf[bitrevtable_i1] & 0xFFFF);
				adc_data_buf2[4] = (uint16_t) ((mag_data_buf[bitrevtable_i1] >> 16) & 0xFFFF);

				// calculate FFT on appropriate buffer
				if (fft_i == (fftSize<<1) - 2 || fft_i2 == (fftSize<<1) - 2) // if we have fftSize/2 new samples
				{
					adc_data_buf2[1] |= 0x8000;
					fft_fsm = (fft_fsm + 1) & 3;
			...
			{
				adc_data_buf1[3] = (uint16_t) (mag_data_buf[bitrevtable_i1] & 0xFFFF);
				adc_data_buf1[4] = (uint16_t) ((mag_data_buf[bitrevtable_i1] >> 16) & 0xFFFF);

				// calculate FFT on appropriate buffer
				if (fft_i == (fftSize<<1) - 2 || fft_i2 == (fftSize<<1) - 2) // if we have fftSize/2 new samples
				{
					adc_data_buf1[1] |= 0x8000;
					fft_fsm = (fft_fsm + 1) & 3;
					// calculate FFT on new chunk
					arm_cfft_q15(S_CFFT, fft_data_buf[to_calc], ifftFlag, doBitReverse);
					arm_cmplx_mag_squared_q15(fft_data_buf[to_calc], mag_data_buf, fftSize);
			...
			if (cl_count == 0)
			{
				if (!ctrl_en2)
				{
					if (ctrl_dir)
					{
						if ((power > ctrl_thresh))
						{
							cl_count = dead_len;
							if (sample_bool)
							{
								adc_data_buf2[2] &= 0x8000;
							}
							else
							{
								adc_data_buf1[2] &= 0x8000;
							}
							if (!fake_stim)
							{
								if (cl0_en)
				...
				else
				{
					if (ctrl_comb)
					{
						...
						{
							if ((power > ctrl_thresh) || (power2 > ctrl_thresh2))
							{
								cl_count = dead_len;
								if (sample_bool)
								{
...
//	accel_init();
    ...
    adc_data_buf1[0] = 0xAA00;
    adc_data_buf2[0] = 0xAA00;
					...
					for(i = 0; i < 66; i++)
					{
						...
						else
						{
							...
							if (sel_data_vector[i-1] != 0)
							{
								if (!stim_flag && (dummy & 0x8000))
									stim_flag = true;
								adc_data_buf2[chan] = ((dummy & 0x7FFF)>>1) + ((adc_data_buf1[chan] & 0x7FFF)>>1);
								adc_data_buf1[chan] = dummy;
								chan++;
							}
						}
					...
					for(i = 0; i < 66; i++)
					{
						...
						else
						{
							...
							if (sel_data_vector[i-1] != 0)
							{
								if (!stim_flag && (dummy & 0x8000))
									stim_flag = true;
								adc_data_buf1[chan] = ((dummy & 0x7FFF)>>1) + ((adc_data_buf2[chan] & 0x7FFF)>>1);
								adc_data_buf2[chan] = dummy;
								chan++;
							}
						}
...
//					}
			...
			if ((crcflag0 & 0x1) && stream_en)
			{
				...
				if (sample_bool)
				{
					if (ch_order)
					{
						fft_cl((int16_t) (adc_data_buf2[1] & 0x7FFF));
					}
					else
					{
						fft_cl((int16_t) (adc_data_buf2[2] & 0x7FFF));
					}
					adc_data_buf2[DATA_BUF_SIZE-1] = counter;
				}
				else
				{
					if (ch_order)
					{
						fft_cl((int16_t) (adc_data_buf1[1] & 0x7FFF));
					}
					else
					{
						fft_cl((int16_t) (adc_data_buf1[2] & 0x7FFF));
					}
					adc_data_buf1[DATA_BUF_SIZE-1] = counter;
				}
				MSS_SPI_set_slave_select( &g_mss_spi0, MSS_SPI_SLAVE_0 );
				if (sample_bool)
					MSS_SPI_transfer_block( &g_mss_spi0, ((uint8_t *) adc_data_buf2) + 1, 2*DATA_BUF_SIZE - 1, spi_rx_buf, sizeof(spi_rx_buf));
				else
					MSS_SPI_transfer_block( &g_mss_spi0, ((uint8_t *) adc_data_buf1) + 1, 2*DATA_BUF_SIZE - 1, spi_rx_buf, sizeof(spi_rx_buf));
				MSS_SPI_clear_slave_select( &g_mss_spi0, MSS_SPI_SLAVE_0 );
				spi_rx_handler();
			}
			else
			...
			if (!stim_flag || !blank_en || artifact_en)
				sample_bool = !sample_bool;

			prev_stim = stim_flag && artifact_en;
		}
